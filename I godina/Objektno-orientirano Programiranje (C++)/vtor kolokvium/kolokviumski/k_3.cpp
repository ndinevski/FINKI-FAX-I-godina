...
public:
    ...
    virtual int getBodovi(){
       ...
       for (int i=0; i<brojOcenki; i++)
            if (ocenki[i]>=6)
                preodni++;
        return preodni*100.0/brojOcenki;
    }
    virtual void pecati(){
        cout<<indeks;
    }
...
class Demonstrator : public Student, public Predavach{
    int brojnacasovi;
public:
    Demonstrator(int indeks,int *ocenki, int brojOcenki, char *imeIPrezime, Kurs *kursevi,int brojKursevi, int brojnacasovi):
    ...
    int getBodovi(){
        ...
        return Student::getBodovi()+20*brojnacasovi/getbrojKursevi();
    }
    void pecati(){
        Student::pecati();
        cout<<": ";
